...
if(isset($_POST['time'])){
        $opc=$_POST['time'];
        if($opc==0){
                echo "ESTADISTICA POR HORA<br>La tabla muestra el promedio por hora de cada dia para el sensor ".$_POST['tipo']."<br>";
	...
	if($opc==1){
		echo "ESTADISTICA POR DIA<br>";
                $sistemas=$conexion->query("select sistema from sensor where tipo='".$_POST['tipo']."'");
                while($row=mysqli_fetch_array($sistemas)){
			$tabla=$conexion->query_table(array("fecha",
				"format((estadistica_hora.00+estadistica_hora.01+estadistica_hora.02+estadistica_hora.03+estadistica_hora.04+estadistica_hora.05+estadistica_hora.06+estadistica_hora.07+estadistica_hora.08+estadistica_hora.09+
					estadistica_hora.10+estadistica_hora.11+estadistica_hora.12+estadistica_hora.13+estadistica_hora.14+estadistica_hora.15+estadistica_hora.16+estadistica_hora.17+estadistica_hora.18+estadistica_hora.19+
					estadistica_hora.20+estadistica_hora.21+estadistica_hora.22+estadistica_hora.23)/24,2) as promedio"),
				array("fecha","promedio"),
				"estadistica_hora",
				"where fecha>='".$_POST['fechaini']."' and fecha<='".$_POST['fechater']."' and tipo='".$_POST['tipo']."' and sistema=".$row['sistema']);
                        $res=$conexion->query("select nombre from sistema where id=".$row['sistema']);
                        $sis=mysqli_fetch_array($res);
                        echo $sis['nombre']."<br>".$tabla;
                        }
	...
	if($opc==2){
		echo "ESTADISTICA POR SEMANA<br>";
                $sistemas=$conexion->query("select sistema from sensor where tipo='".$_POST['tipo']."'");
                while($row=mysqli_fetch_array($sistemas)){
	                $sql="select fecha from estadistica_hora where fecha>='".$_POST['fechaini']."' and fecha<='".$_POST['fechater']."' and tipo='".$_POST['tipo']."' and sistema=".$row['sistema'];
                        $result=$conexion->query($sql);
                        $tabla="<table border> <tr><th>SEMANA</th><th>PROMEDIO</th></tr>";
			$query="select 
				format(avg((estadistica_hora.00+estadistica_hora.01+estadistica_hora.02+estadistica_hora.03+estadistica_hora.04+estadistica_hora.05+estadistica_hora.06+estadistica_hora.07+estadistica_hora.08+estadistica_hora.09+
                                        estadistica_hora.10+estadistica_hora.11+estadistica_hora.12+estadistica_hora.13+estadistica_hora.14+estadistica_hora.15+estadistica_hora.16+estadistica_hora.17+estadistica_hora.18+estadistica_hora.19+
                                        estadistica_hora.20+estadistica_hora.21+estadistica_hora.22+estadistica_hora.23)/24),2) as promedio
				from estadistica_hora where tipo='".$_POST['tipo']."' and sistema=".$row['sistema']." and fecha in (";
			$count=0;
				...
				if($mes!=substr($row2['fecha'],5,2)){
					...
					$query="select 
						format(avg((estadistica_hora.00+estadistica_hora.01+estadistica_hora.02+estadistica_hora.03+estadistica_hora.04+estadistica_hora.05+estadistica_hora.06+estadistica_hora.07+estadistica_hora.08+estadistica_hora.09+
                                	        estadistica_hora.10+estadistica_hora.11+estadistica_hora.12+estadistica_hora.13+estadistica_hora.14+estadistica_hora.15+estadistica_hora.16+estadistica_hora.17+estadistica_hora.18+estadistica_hora.19+
                                	        estadistica_hora.20+estadistica_hora.21+estadistica_hora.22+estadistica_hora.23)/24),2) as promedio
						from estadistica_hora where tipo='".$_POST['tipo']."' and sistema=".$row['sistema']." and fecha like '_____".$mes."___'";
					$rprom=$conexion->query($query);
        	                        $riw=mysqli_fetch_array($rprom);
                        	        $tabla.="<td>".$riw['promedio']."</td></tr>";
                                	}
