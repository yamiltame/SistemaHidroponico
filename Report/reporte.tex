...
tambi\'en nos referiremos a \'el como ''scheduller''. El planificador de tareas es un programa que mantiene los vegetales regados y procesa las variables de entorno para 
llevar acabo los procesos que 
las mantengan en el rango ideal. Cada cultivo tiene un rago ideal de ph, temperatura ambiente, humedad ambiente y temperatura en la soluci\'on de riego; definidos 
en la 
...
conectados a los pines de la tarjeta. El planificador de tareas, igual que todos los programas, fue programado y compilado en el microprocesador; los c\'odigos se 
...
Antes de hacer la instalaci\'on de los paquetes se debe ejecutar el comando

\begin{verbatim}
 sudo apt-get update
\end{verbatim}

Una vez instalados los paquetes es necesario reiniciar el duino. Al reiniciar se nota que el servidor apache no 
...
preferencias y luego pasar a las conexiones de red donde se edita la conexi\'on que se utilizar\'a. Para este caso se utiliza la conexi\'on por cable ethernet. 
Para fijar la direcci\'on ip se debe editar la conexi\'on pasando a 
la pesta\~na "ipv4" (internet protocol 
version 4) y a\~nadiendo los datos correspondientes. Actualmente el Pcdino tiene la siguiente configuraci\'on:
...
del driver. Una vez agregados los archivos, se compila el driver ejecutando el binario \verb|comp| dentro de la carpeta \verb|Hidroponia| ingresando el comando:

\begin{verbatim}
./comp
\end{verbatim}

El binario \verb|comp| se genera del c\'odigo \verb|compiler.cpp| ubicado en la carpeta Codes. Para crear el binario, es necesario ejecutar dentro de la carpeta 
...
todos los archivos \verb|driver.cpp| que se encuentren en la carpeta \verb|Drivers| y a partir de ellos genera los archivos \verb|driver.o| los cuales contienen las 
funciones declaradas; estos 
son enviados a la carpeta \verb|Hidroponia/Objects|. Los objetos sirven para crear los ejecutables que usen los drivers. Una vez que est\'an hechos los 
...
La temperatura y la humedad ambiente nos las provee un s\'olo sensor, es el sensor DHT22 de arduino. El sensor es digital, por lo que ahora no se utilizan los puertos adc, 
se usan los puertos GPIO. Lo que hace el driver es fijar el pin en modo output para despu\'es 
capturar los bytes y procesarlos. El procesamineto de bytes devuelve lecturas de temperatura en celcius y humedad relativa en porcentaje (de 100). Es un s\'olo driver 
pero se crearon dos funciones, 
...
El sensor de humedad en tierra es parecido al de ph, es del formato adc, consiste en 2 placas met\'alicas conectadas a una diferencia de potencial, cuando est\'an 
...
asociamos el 0\% de humedad, por lo tanto tenemos 2 puntos $(V_w,100\%)$ y $(V_a,0\%)$ con los cuales se hace un ajuste lineal (una recta que pase por los 
dos) y obtenemos:
\begin{equation}
...
La base de datos nos sirve para registrar toda la informaci\'on de los 
...
Para la informaci\'on de los sistemas tenemos varias tablas. La tabla 
\verb|sistema| guarda la informaci\'on del sistema como nombre del sistema, cultivo, id del 
sistema, url, ubicaci\'on y si es sistema principal o subsistema. La 
...
valores de temperatura del agua; cada uno relacionado con la temperatura ambiente, la relaci\'on se explicar\'a m\'as adelante. La tabla 
...
refiere a encendio o apagado, 0 apagado y 1 encendido.
...
Dentro de los campos de las horas del d\'ia se guardar\'a el promedio de las lecturas que se hayan efectuado a esa hora, por ejemplo, un registro quedarr\'ia de la 
...
actualiza mediante un trigger que se activa al introducirce un nuevo registro en la tabla \verb|monitoreo_sensor|. 
...
\begin{itemize}
  ...
  \item \verb|muestreo|: los minutos entre cada muestreo (periodo de muestreo). 
  \item \verb|pin|: el pin al que est\'e conectado el sensor.
  \item \verb|sys|: el identificados del sistema al que pertenece el sensor.
  \item \verb|valor|: es el valor de la variable de entorno que representa el sensor. 
  \item \verb|name|: es el tipo de sensor, sirve para relacionar el sensor con el driver. 
\end{itemize}

La estructura \verb|actuador| 
es an\'aloga a \verb|sensor| para las variables de \verb|id|, \verb|pin|, \verb|sys| y \verb|name|. Las variable diferente \verb|seg|, representa los segundos de acci\'on 
...
rudimentaria, ser\'ia buena idea trabajar en este aspecto del proyecto. El ejecutable no tiene manera de hacer m\'ultiples pruebas o de suspender la ejecuci\'on de una 
...
un actuador de tipo calefacor y un sensor de temperatura en liquidos; el principio es el mismo que el segundo riego, si el agua baja a cierta temperatura se activa el 
calefactor 
...
actuadores, para subir el ph ser\'ia
...
igual para disminuir el ph pero ser\'ia \verb|while(sensorph.valor>phmax)|. La letra \verb|t| representa un peque\~no periodo de muestreo para saber cuando desactivar 
...
En este ejemplo el thread espera en el loop \verb|while| hasta que se libera el pin 4, saliendo del loop se fija el valor en true para establecerlo como ocupado; 
...
corriendo un thread mediante su tid, se apoya de una carpeta que contiene archivos con informaci\'on de status para cada thread que sea ejecutado; la carpeta es 
...
La opci\'on 2 lo que hace es actualizar los ''schedullers'' en ejecuci\'on con la base de datos. El pseudoc\'odigo es el siguiente:
...
Por lo tanto si quisieramos apagar o encender un determinado sistema lo 
recomendable 
ser\'ia editar el campo \verb|estado| en la base de datos y ejecutar la opci\'on 2 del administrador de sistemas.

\subsection{Opci\'on 3}
Esta opci\'on sigue en desarrollo, se ha comentado a lo largo del documento, sirve para automatizar el protocolo para a\~nadir un driver al sistema. Lo que se tiene hasta 
...
Se cre\'o una peque\~na interfaz gr\'afica provicional para agregar/editar sistemas, planificadores y riegos; tambi\'en 
nos permite monitorear las lecturas de los sensores y las acciones de 
...
An\'alogo a sistemas, se muestran allas opciones de crear o de editar planificaciones y riegos Los c\'odigos de las p\'aginas son \verb|planificador.php| y \verb|riego.php| 
respectivamente.

\subsection{Monitoreo}
Es en el archivo \verb|monitoreo.php|, lo que hace el script es filtrar las tablas \verb|monitoreo_sensor| y \verb|monitoreo_actuador| pregunt\'andole al 
