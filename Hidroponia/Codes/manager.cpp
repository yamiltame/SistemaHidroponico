...
void *launchremote(void *arg){
	...
	string cmd="bash -c 'exec -a "+sistema->name+" ./Hidroponia "+to_string(sistema->system)+"'";
	system(cmd.c_str());
	pthread_exit(NULL);
	}
...
void *launchwateradmin(void *arg){
	if(!running("wateradmin")){
		cout<<"creating wateradmin"<<endl;
		string cmd="bash -c 'exec sudo nice --7 ./wateradmin'";
		system(cmd.c_str());
		}
	pthread_exit(NULL);
	}

void *launchstats(void *arg){
	if(!running("stats")){
		cout<<"creating stats"<<endl;
		string cmd="bash -c 'exec sudo ./stats'";
		system(cmd.c_str());
		}
	pthread_exit(NULL);
	}
...
void kill_todo(){
	...
	if(running("stats")){system("sudo pkill stats");}
	}
...
void load_systems(){
	...
	rc = pthread_create(&statistics, NULL, launchstats, NULL);
        if (rc) {
        	cout << "Error:unable to create thread," << rc << endl;
                exit(-1);
                 }
...
//        string file="Logs/adminlog";
//        string errfile="Logs/Error/adminerr";
