...
void *monitoreo(void *arg){
	...
	cout<<"thread monitoreo creado con tid: "<<Hilos[0].tid<<endl;
	conector cc;
	sleep(10);
	while(1){
		if(!consultas.empty()){
	...
	cout<<"thread monitoreo morira"<<endl;
	pthread_exit(NULL);
	}
...
void *getValue(void *arg){
	string *type;
	type=(string *)arg;
	if(*type=="temperatura_ambiente"){Hilos[1].tid=syscall(SYS_gettid);
	cout<<"thread para obtener valor global "<<*type<<" con tid:"<<Hilos[1].tid<<endl;
		}
	else{Hilos[2].tid=syscall(SYS_gettid);
	cout<<"thread para obtener valor global "<<*type<<" con tid:"<<Hilos[2].tid<<endl;
		}
	...
	while(1){
		...
		if(*type=="temperatura_ambiente"){
			//cout<<"temperatura ambiente: "<<temp_amb<<"°C"<<endl;
			setriego();
			}
		if(*type=="humedad_ambiente"){
			//cout<<"humedad ambiente: "<<hum_amb<<"%"<<endl;
			}
		sleep(muestreo*60);
		}
	}
...
void *sensar(void *arg){
	...
	cout<<"thread sensor "<< S->name<<" creado con tid: "<<Hilos[c++].tid<<endl;
	string sql;
	int attempts,s;
	while(1){
		aguanta(S->pin);
		...
		while(S->valor==-1){
			if(++attempts>4){
				//intento para monitorear el funcionamiento de los sensores
				//cout<<"Sensor "<< S->name <<" del sistema "<<Stm.name<<" no sirve\n";
				//pthread_exit(NULL);
				}
...
bool running(pid_t tid,string name){
        ...
        tidfile="/home/Hidroponia/threads/"+name;
...
int main(int argc,char* argv[]){
	...
	string errfile="Logs/errfilesys"+to_string(sys);
...
//	cout.rdbuf(out.rdbuf());
	...
	sleep(3);
	for(int j=0;j<7+numsens;j++){cout<<"Tids["<<j<<"]= "<<Tids[j]<<endl;}
	//main loop
	while(1){}
	pthread_exit(NULL);
	return 0;
